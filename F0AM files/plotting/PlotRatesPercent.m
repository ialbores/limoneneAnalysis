function SpRates = PlotRatesPercent(Spname,S,n2plot,varargin)
% function SpRates = PlotRatesPercent(Spname,S,n2plot,varargin)
% Generates a plot of production and loss rates divided by total production/loss rates vs time for a single species.
...
% 2022 ISA  modified to plot percentage rate
...
switch ptype
    case 'fill'
        if (~isnan(P2plot))
            area(ax1,Time,P2plot); end
        if (~isnan(L2plot))
            area(ax2,Time,L2plot); end
        ylimit = max([max(abs(sum(L2plot,2))) max(sum(P2plot,2)) max(abs(dil))]);
    case 'line'
        if (~isnan(P2plot))
            plot(ax1,Time,P2plot,'LineWidth',2); end
        if (~isnan(L2plot))
            plot(ax2,Time,L2plot,'LineWidth',2); end
        plot([min(Time) max(Time)],[0 0],'k','LineWidth',2)
        ylimit = max([max(max(abs(L2plot))) max(max(P2plot)) max(abs(dil))]);
    case 'bar'
        if (~isnan(P2plot))
            bar(ax1,Time,P2plot,'stack'); end
        if (~isnan(L2plot))
            bar(ax2,Time,L2plot,'stack'); end
        ylimit = max([max(abs(sum(L2plot,2))) max(sum(P2plot,2)) max(abs(dil))]);
end
...
ylabel(ax1,[Spname ' Rate Percentage'])
xlabel(ax1,'Model Time')

% HOTP modification due to lack of stats package
%tspacing = nanmean(diff(Time));
tspacing= sum((isfinite(diff(Time)).*diff(Time)),1)/sum((isfinite(diff(Time)).*1),1);
% End HOTP modification
